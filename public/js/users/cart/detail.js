import { formatMoney, renderBanks, renderToast, validatePhoneNumber } from "../../helper.js";
import {
    ADDRESS_UPDATE,
    CART_HISTORY,
...
let addressTree = [];
const cartBody = $("#cart-body-pro"),
    cartTotal_ = $(".order-summary-emphasis:first"),
    cartTotal = $(".payment-due-price");
const mainContentDetails = $(".main-content#details"),
    mainContentPayment = $(".main-content#payment");
...
const renderEleCart = (id, image, name, quantity, type, price, sale) => {
    ...
    <tr class="product" data-id="${id}">
        ...
        <td class="product-price">
            <span class="order-summary-emphasis">${formatMoney(
                ((price * (100 - sale)) / 100) * quantity
            )}₫</span>
        </td>
    </tr>
    `);
    ...
    .on("submit", function (e) {
        ...
        products.each((i, e) => {
            const quantity = parseInt(
                $(e).find(".product-thumbnail-quantity").html()
            );
        ...
        if (!validatePhoneNumber($('#phone').val().trim())) {
            renderToast({
                status: "danger",
                title: "Lỗi",
        ...
        const fullAddress = `${address}, ${$(
            `#customer_shipping_ward option[value="${ward}"]`
        ).html()}, ${$(
        ...
        data.append(
            "payment",
            $('input[name="payment_method_id"]:checked').val()
        );
...
const main = async () => {
    ...
    $("#update_address")
        .off("click")
        .on("click", function () {
            if (confirm("Bạn muốn thay đổi địa chỉ?")) {
                updateAddress();
            }
        });
